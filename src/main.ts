...
(() => {
  ...
  const elements: NodeListOf<HTMLElement> = document.querySelectorAll('.reveal-up');
  ...
  groups.forEach((group) => {
    ...
    children.forEach((child, idx) => child.style.setProperty('--delay', `${idx * 0.06}s`));
  });
})();
...
(() => {
  ...
  const cards = document.querySelectorAll<HTMLElement>('.projects .card[data-repo]');
...
// Card tilt interaction (desktop/hover devices only)
(() => {
